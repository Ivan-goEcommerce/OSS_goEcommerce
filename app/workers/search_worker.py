...
class SearchWorker(QThread):
    ...
    def __init__(self, search_taric, table_name="products", use_n8n=True, n8n_config=None):
        ...
        if use_n8n:
            try:
                ...
                debug_print("Erstelle n8n Workflow-Manager...")
                ...
                if not workflow_url:
                    debug_print("FEHLER: Keine n8n Workflow URL in der Konfiguration gefunden!")
                ...
                debug_print("OK: n8n Workflow-Manager erfolgreich erstellt!")
                debug_print(f"   Workflow URL: {workflow_url}")
                debug_print(f"   Lizenznummer: {license_number}")
                debug_print(f"   E-Mail: {email}")
            except ImportError as e:
                debug_print(f"FEHLER: n8n Workflow Manager nicht verfügbar: {e}")
                self.error_occurred.emit(f"n8n Workflow Manager nicht verfügbar: {e}")
            except Exception as e:
                debug_print(f"FEHLER: n8n Workflow-Verbindung fehlgeschlagen: {str(e)}")
                self.error_occurred.emit(f"n8n Workflow-Verbindung fehlgeschlagen: {str(e)}")
        else:
            debug_print(f"FEHLER: n8n nicht verfügbar. Verwende n8n: {use_n8n}")
    ...
    def run(self):
        ...
        if self.use_n8n and self.n8n_manager:
            try:
                debug_print(f"DEBUG: Führe n8n Workflow für TARIC-Suche aus: {self.search_taric}")
                success, results, message = self.n8n_manager.search_taric_codes(self.search_taric)
                
                debug_print(f"DEBUG: n8n Workflow Response - Success: {success}")
                debug_print(f"DEBUG: n8n Workflow Response - Message: {message}")
                debug_print(f"DEBUG: n8n Workflow Response - Results Type: {type(results)}")
                debug_print(f"DEBUG: n8n Workflow Response - Results: {results}")
                
                if success:
                    if results:
                        debug_print(f"DEBUG: n8n Workflow erfolgreich - Ergebnisse: {len(results) if results else 0} Einträge")
                        # Stelle sicher, dass results eine Liste ist
                        if not isinstance(results, list):
                            results = [results]
                        self.result_ready.emit(results)
                    else:
                        debug_print("DEBUG: n8n Workflow erfolgreich aber keine Ergebnisse")
                        self.result_ready.emit([])
                else:
                    debug_print(f"DEBUG: n8n Workflow Fehler: {message}")
                    self.error_occurred.emit(f"n8n Workflow Fehler: {message}")
            except Exception as e:
                debug_print(f"DEBUG: Exception in n8n Workflow: {str(e)}")
            ...
            debug_print(f"DEBUG: Lokale Suche für TARIC-Code: {self.search_taric}")
